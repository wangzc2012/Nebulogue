...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | ✨Nebulogue")[0];
}
...
function createtime1() {
  var grt = new Date("07/15/2025 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Wangzc2012 %c 你正在访问Wangzc2012の小家",
...
/* 夜间模式切换动画 start */
var themeAnimationClickHandler = null;
function switchNightMode() {
  document.querySelector('body').insertAdjacentHTML('beforeend', '<div class="Cuteen_DarkSky"><div class="Cuteen_DarkPlanet"><div id="sun"></div><div id="moon"></div></div></div>');
  var darkSky = document.getElementsByClassName('Cuteen_DarkSky')[0];
  var skipAnimation = function() {
    if (darkSky && darkSky.parentNode) {
      darkSky.style.transition = 'opacity 0.3s';
      darkSky.style.opacity = '0';
      setTimeout(function() {
        darkSky.remove();
      }, 300);
    }
    if (themeAnimationClickHandler) {
      document.removeEventListener('click', themeAnimationClickHandler);
      themeAnimationClickHandler = null;
    }
  };
  themeAnimationClickHandler = skipAnimation;
  document.addEventListener('click', themeAnimationClickHandler, {once: true});
  setTimeout(function () {
    if (themeAnimationClickHandler) {
      document.removeEventListener('click', themeAnimationClickHandler);
      themeAnimationClickHandler = null;
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("07/15/2025 00:00:00");
...
function toggleWinbox() {
  ...
  if (box && winbox && typeof winbox.hide === 'function') {
    if (box.style.display === 'none' || box.classList.contains('hide')) {
      winbox.show();
      if (!winboxEscHandler) {
        winboxEscHandler = function(e) {
          if (e.key === 'Escape') {
            winbox.hide();
          }
        };
        document.addEventListener('keydown', winboxEscHandler);
      }
    } else {
      winbox.hide();
      if (winboxEscHandler) {
        document.removeEventListener('keydown', winboxEscHandler);
        winboxEscHandler = null;
      }
    }
  } else {
    createWinbox();
